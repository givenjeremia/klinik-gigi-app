...
try {
    ...
    $sql ="SELECT rk.id AS IdReservasi,group_concat(DISTINCT k.nama ORDER BY k.nama DESC SEPARATOR ', ') as Keluhan, rk.jadwal_dokter_id as IdJadwal , jd.kuota_pasien as KuotaPasien ,dd.nama AS NamaDokter, dp.nama AS NamaPasien, rk.jam_reservasi AS Jam, rk.tanggal_reservasi AS Tanggal, rk.status as Status
    FROM `reservasi_kllinik` rk INNER JOIN `data_pasien` dp ON rk.data_pasien_id_pasien = dp.id
    INNER JOIN `jadwal_dokter` jd ON jd.id = rk.jadwal_dokter_id
    INNER JOIN `data_dokter` dd ON jd.data_dokter_id = dd.id
