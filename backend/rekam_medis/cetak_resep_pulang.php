...
$no=1;
$data = mysqli_query($mysqli,"SELECT ro.*,od.*, ro.harga as HargaTotal FROM `resep_obat` ro INNER JOIN `data_obat` od ON ro.data_obat_id = od.id WHERE `status`=1 AND `status_kesediaan`=0 AND `rekam_medis_id` = $id_rekam_medis");
